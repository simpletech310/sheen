...
export function BookingVehicleList({
  ...
  const accent = dark ? "border-sol" : "border-sol";
  const body = dark ? "text-bone/85" : "text-ink/85";
  const photoBg = dark ? "bg-white/10 border-white/15 hover:border-bone" : "bg-mist border-mist hover:border-ink";
            ...
            {r.condition_photo_paths.length > 0 && (
              <div className="mt-3">
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {r.condition_photo_paths.map((path) => {
                    ...
                    return (
                      <a
                        ...
                        className={`block aspect-square overflow-hidden border transition ${photoBg}`}
                      >
                        {url ? (
