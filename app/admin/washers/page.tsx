...
export default async function AdminWashersPage() {
  ...
  const { data: washers } = await supabase
    ...
    .select(
      "user_id, status, jobs_completed, rating_avg, stripe_account_id, insurance_doc_url, background_check_verified, users(email, full_name)"
    )
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-mist text-left">
              ...
              <th className="px-4 py-3 font-mono text-[10px] uppercase text-smoke">Background</th>
            ...
            {(washers ?? []).map((w: any) => (
              <tr key={w.user_id} className="border-b border-mist last:border-b-0">
                ...
                <td className="px-4 py-3">
                  <VerifyButton userId={w.user_id} verified={!!w.background_check_verified} />
                </td>
                <td className="px-4 py-3">
